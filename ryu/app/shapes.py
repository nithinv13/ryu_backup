...
class algo_runner:
	...
	def initialize(self, switches, hosts, dpids, sw_coor, ho_coor):
		...
		self.dpids = copy.deepcopy(dpids)
	...
	def print_specifics(self, sh, dhs, algorithm, output):
		index = 100
		lister = []
		lister += [{"sh":sh}]
		lister += [{"dhs":dhs}]
		lister += [{"algorithm":algorithm}]
	...
	def show_trees_from_final_trees(self, sh, dhs, switches, hosts, dpids, sw_coor, ho_coor, tree, branch_state_nodes):
		...
		for src in tree:
			...
			if src.find("h") == -1:
				...
				x1 = self.sw_coor[dst]["x"]
				y1 = self.sw_coor[dst]["y"]
				x2 = self.ho_coor[src]["x"]
				y2 = self.ho_coor[src]["y"]
			self.canvas.create_line(x1, y1, x2, y2, fill = "red", width = 3)
	...
	def find_sw_in_each_level(self):
		for sw in self.dpids:
			if self.dpids[sw]["level"] == 0:
				self.l0_switches += 1
			elif self.dpids[sw]["level"] == 1:
				self.l1_switches += 1
			...
			for dst in self.switches[src]:
				if src.find("s")!=-1 and dst.find("s")!=-1:
					if included[dst][src] != 1:
						...
						if x1 == x2:
							self.canvas.create_text((x1+x2)/2-5,(y1+y2)/2, text = "%s"%(self.switches[src][dst]["cost"]), fill = "black")
						elif x1 > x2 :
							self.canvas.create_text((x1+x2)/2+5,(y1+y2)/2+5, text = "%s"%(self.switches[src][dst]["cost"]), fill = "black")
						else:
							self.canvas.create_text((x1+x2)/2-5,(y1+y2)/2+5, text = "%s"%(self.switches[src][dst]["cost"]), fill = "black")
						included[src][dst] = 1
