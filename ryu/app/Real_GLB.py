...
def RandUpperNeigh1(dst, graph):
	global dpids
	print "in rand upper neigh 1"
...
def RandUpperNeigh(dst, graph):
	...
	adj_nodes = list(graph[dst])
	#print adj_nodes
	temp_adj_nodes = copy.deepcopy(adj_nodes)
	for node in temp_adj_nodes:
		#print node, dpids[node]["level"]
		if dpids[node]["level"] <= dpids[dst]["level"]:
			adj_nodes.remove(node)
	#print adj_nodes
	min_cost = sys.maxint
	min_node = None
	for node in adj_nodes:
		if switches[dst][node]["cost"] < min_cost:
			min_cost = switches[dst][node]["cost"]
			min_node = node
	return node
...
def BFS(dst, tree, graph, prev):
	...
	paths = list(dfs_paths(src, dst, dist))
	min_cost = sys.maxint
	min_path = None
	for path in paths:
		cost = cal_path_cost(path)
		if cost < min_cost:
			min_cost = cost
			min_path = path
	for i in range(len(min_path)-1):
		prev[min_path[i+1]] = min_path[i]
...
def dfs_paths(start, goal, path_len):
    ...
    while stack:
        ...
        #print vertex
        #print cost
        #print list(graph[vertex]-set(path))
        for next in graph[vertex] - set(path):
            if next == goal and cost + 1 == path_len:
                yield path + [next]
            elif cost+1 < path_len:
                stack.append((next, path + [next], cost + 1))

def cal_path_cost(path):
	cost = 0
	for i in range(0, len(path)-1):
		cost += switches[path[i]][path[i+1]]["cost"]
	return cost

def get_shortest_traffic_path(src, dst):
	prev, dist = shortest_paths(src)
	path_len = dist[dst]
	paths = list(dfs_paths(src, dst, path_len))
	min_cost = sys.maxint
	min_path = None
	for path in paths:
		cost = cal_path_cost(path)
		if cost < min_cost:
			min_cost = cost
			min_path = path
	return copy.deepcopy(min_path), min_cost

def extended_bfs(src, dst, tree, prev):
	global switches
	seen_switches = set()
	queue1 = []
	queue2 = []
	...
	while found == 0:
		if odd == 1:
			...
			for entry in queue1:
				print entry
				for sw in switches[entry[0]]:
					print sw
					if sw in seen_switches:
						print "continuing"
						continue
					if sw.find("h") != -1:
						continue
					path, cost = get_shortest_traffic_path(dst, sw)
					queue2 += [(sw, path, cost)]
				seen_switches.add(entry[0])
			queue2 = sorted(queue2, key= lambda x:x[2])
			print "Entries in queue 2"
			for entry in queue2:
				print entry
			print "next_time"
			for entry in queue2:
				if entry[0] in tree and entry[0].find("h")==-1:
					found = 1
					req_path = entry[1]
					for i in range(len(req_path)-1):
						prev[req_path[i]] = req_path[i+1]
					for node in req_path:
						if node not in tree:
							tree.add(node)
			...
			for entry in queue2:
				for sw in switches[entry[0]]:
					...
					queue1 += [(sw, path, cost)]
				seen_switches.add(entry[0])
			queue1 = sorted(queue1, key= lambda x:x[2])
			print "Entries in queue 1"
			for entry in queue1:
				print entry
			print "next_time"
			for entry in queue1:
...
def tree_constructor(sh, dhs):
	...
	dst = max_dist_dh
	print "max_dist_dh is " + str(max_dist_dh)
	tree.add(dst) 
	path_len = dist[max_dist_dh]
	paths = list(dfs_paths(sh, max_dist_dh, path_len))
	min_cost = sys.maxint
	min_path = None
	for path in paths:
		cost = cal_path_cost(path)
	...
	while pre is not None:
		...
		cur = pre
		pre = prev[cur]
	print "The tree nodes are"
	print tree

	dhs_temp = copy.deepcopy(dhs)
	dhs_temp.remove(max_dist_dh)
	...
	for dh in dhs:
		...
		covered_set[dst] = True
		zero = 0
		tree1, prev1 = extended_bfs(src, dst, tree, prev)
...
def increment_switch_cost(tree, flow_rate):
	for src in tree:
		dest = tree[src]
		if dest != None:
...
def tree_ports_all(tree_no, sh, dhs, algorithm, flow_rate):
	...
	increment_switch_cost(tree, flow_rate)
